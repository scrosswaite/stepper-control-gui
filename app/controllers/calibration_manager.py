...
class CalibrationManager(QObject):
    def __init__(self, serial, update_progress, update_depth, set_status, set_leds, unlock_ui_callback, steps_per_rev=200, lead_mm=4.0, click_mm=0.5):
        ...
        # Mechanical params that can be set from MainWindow
        ...
        # Calibration parameters set by the dialog
        self._interval_ms = 10000 # Default to 10 seconds
    ...
    def start(self):
        """Starts the calibration sequence with the currently set parameters."""
        self._calib_step = 0
        self._calib_total_mm = 0.0
        self._stop_requested = False
        ...
        QTimer.singleShot(2000, self._start_timer) # Delay for homing to complete

    def _start_timer(self):
        """Initializes and starts the timers for the sequence."""
    ...
    def _run_next_calib_step(self):
        ...
        if self._calib_step >= 10:
            ...
            self.serial.send("HOME\n")
            QTimer.singleShot(2000, self._finish) # Delay for homing
